...
class JsonModelViewer3D {
  constructor(containerId = 'viewer') {
    ...
    // lights (simple but clear)
    this.scene.add(new THREE.AmbientLight(0xffffff, 0.7))
    const key = new THREE.DirectionalLight(0xffffff, 0.9); key.position.set(2.5, 3, 4); this.scene.add(key)
    const rim = new THREE.DirectionalLight(0xffffff, 0.4); rim.position.set(-2, 1.5, -3); this.scene.add(rim)
    ...
    // material tri-state
    this.materialBase   = new THREE.MeshStandardMaterial({ color: '#9ea3a8', metalness: 0.2, roughness: 0.35, side: THREE.DoubleSide })
    this.materialHover  = new THREE.MeshStandardMaterial({ color: '#2d6cff', metalness: 0.2, roughness: 0.35, side: THREE.DoubleSide })
    this.materialSelect = new THREE.MeshStandardMaterial({ color: '#ff3b3b', metalness: 0.2, roughness: 0.35, side: THREE.DoubleSide })
    ...
    this._down = {x:0,y:0}
    this.renderer.domElement.addEventListener('mousedown', (e)=>{ this._down.x=e.clientX; this._down.y=e.clientY })
  ...
  loadJsonData(json) {
    ...
    if (!mesh) { console.warn('[JsonModelViewer3D] unsupported/empty JSON'); return }
  ...
  buildEdges() {
    if (this.edgesLine) {
      ...
      this.edgesLine.geometry?.dispose(); this.edgesLine.material?.dispose()
      this.edgesLine = null
    }
    if (!this.mesh) return
  ...
  applyMaterialPreset(preset) {
    ...
    this.materialHover.metalness = p.metalness; this.materialHover.roughness = p.roughness
    this.materialSelect.metalness = p.metalness; this.materialSelect.roughness = p.roughness
  ...
  ensureMeasureLabel() {
    ...
    Object.assign(el.style, { position:'absolute', padding:'4px 8px', background:'rgba(124,58,237,.95)', color:'#fff', borderRadius:'6px', fontSize:'12px', pointerEvents:'none', transform:'translate(-50%, -120%)' })
  ...
  setMeasureMode(enabled){ this.measureMode = !!enabled; if(!enabled) this.resetMeasure(); this.renderer.domElement.style.cursor = enabled?'crosshair':'default' }
  resetMeasure(){
    this.measurePoints = []; this.measurePreviewPoint = null
    if (this.measureLine) { this.scene.remove(this.measureLine); this.measureLine.geometry?.dispose(); this.measureLine=null }
    if (this.measureLabelEl) { this.measureLabelEl.remove(); this.measureLabelEl=null }
  }
  updateMeasureVisual(){
    ...
    if (!p1 || !p2) { if (this.measureLine) { this.scene.remove(this.measureLine); this.measureLine.geometry?.dispose(); this.measureLine=null } ; if(this.measureLabelEl){this.measureLabelEl.style.display='none'}; return }
    const geo = new THREE.BufferGeometry()
    geo.setAttribute('position', new THREE.Float32BufferAttribute([p1.x,p1.y,p1.z,p2.x,p2.y,p2.z],3))
    if (!this.measureLine) { this.measureLine = new THREE.Line(geo, this.measureMaterial); this.scene.add(this.measureLine) } else { this.measureLine.geometry.dispose(); this.measureLine.geometry = geo }
    ...
    this.measureLabelEl.style.left = `${s.x}px`; this.measureLabelEl.style.top = `${s.y}px`; this.measureLabelEl.style.display='block'
  }

  // --- Builders ---
  buildMeshFromOnshapeJson(json) {
    const bodies = json?.faces?.bodies; if (!Array.isArray(bodies)) return null
  ...
  buildMeshFromFreecadJson(json) {
    const objects = json?.objects; if (!Array.isArray(objects)) return null
  ...
  fitCamera() {
    ...
    this.camera.position.copy(center.clone().add(new THREE.Vector3(dist, dist, dist)))
  ...
  onPointerMove(e) {
    ...
    if (hits[0]?.faceIndex != null && Array.isArray(this.mesh.geometry.groups)) {
      ...
      for (let gi = 0; gi < this.mesh.geometry.groups.length; gi++) {
        ...
        if (triIndex >= triStart && triIndex < triEnd) { newHover = gi; break }
      }
    }
  ...
  onMouseUp(e) {
    ...
    if (hit.faceIndex != null && Array.isArray(this.mesh.geometry.groups)) {
      ...
      for (let gi = 0; gi < this.mesh.geometry.groups.length; gi++) {
        ...
        const triStart = g.start / 3
        const triEnd = (g.start + g.count) / 3
        if (triIndex >= triStart && triIndex < triEnd) { groupIdx = gi; break }
      }
    }
    ...
    for (let i = fg.start; i < fg.start + fg.count; i++) { sx += pos.getX(i); sy += pos.getY(i); sz += pos.getZ(i); n++ }
    ...
    for (let i = fg.start; i < fg.start + fg.count; i+=3) {
      ...
      ab.subVectors(b,a); ac.subVectors(c,a); areaM2 += ab.cross(ac).length()*0.5
    }
    const detail = {
      id: faceId,
      realFaceId: realId,
...
function ensureViewer() { if (!JSON_VIEWER) JSON_VIEWER = new JsonModelViewer3D('viewer'); return JSON_VIEWER }
...
window.addEventListener('viewer-fit', () => { /* no-op: orbit auto fits on load */ })

Livewire.on('toggleShowEdges', ({ show, threshold = null, color = null }) => {
  const v = ensureViewer(); v.toggleEdges(show, threshold, color)
})
Livewire.on('updatedMaterialPreset', ({ preset = null, color = null, metalness = null, roughness = null }) => {
  ...
  if (color) { v.materialBase.color.set(color); v.updateMaterialStates() }
  if (Number.isFinite(metalness)) { v.materialBase.metalness = +metalness; v.updateMaterialStates() }
  if (Number.isFinite(roughness)) { v.materialBase.roughness = +roughness; v.updateMaterialStates() }
})
Livewire.on('toggleMeasureMode', ({ enabled }) => { const v = ensureViewer(); v.setMeasureMode(!!enabled) })
Livewire.on('resetMeasure', () => { const v = ensureViewer(); v.resetMeasure() })
